...
def user_register(request):
    if request.method == 'POST':
        ...
        if password2!=password:
            return render(request,'user-register.html',{'msg':"password doesn't match"})

        if CustomUser.objects.filter(username=username).exists():
            return render(request, 'user-register.html', {'msg': "Username already exists"})
        if CustomUser.objects.filter(email=email).exists():
            return render(request, 'user-register.html', {'msg': "Email already exists"})
        if CustomUser.objects.filter(phone_number=phone).exists():
            return render(request, 'user-register.html', {'msg': "Phone number already exits"})

        data = CustomUser.objects.create_user(username=username, email=email, password=password, phone_number=phone, user_type="user", status="approve")
        data.save()
...
def agency_register(request):
    if request.method == 'POST':
        ...
        phone = request.POST['phone']
        if password2!=password:
            return render(request,'agency-register.html',{'msg':"password doesn't match"})

        if CustomUser.objects.filter(username=username).exists():
            return render(request, 'agency-register.html', {'msg': "Username already exists"})
        if CustomUser.objects.filter(email=email).exists():
            return render(request, 'agency-register.html', {'msg': "Email already exists"})
        if CustomUser.objects.filter(phone_number=phone).exists():
            return render(request, 'agency-register.html', {'msg': "Phone number already exits"})

        data = CustomUser.objects.create_user(username=username,agencyName=agencyName, email=email, password=password, phone_number=phone, user_type="agency")
        data.save()
        return redirect(Login)
    else:
...
def edit_agencyprofile(request):
    ...
    if request.method == 'POST':
        ...
        User.agencyName = request.POST['agencyName']
        User.save()
        return redirect(agencyindex)
    else:
        context = {
